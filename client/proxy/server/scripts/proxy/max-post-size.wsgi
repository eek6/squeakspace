...
def post_handler(environ):
    ...
    try:
        c = db.cursor(conn)
        resp = db.change_max_post_size(c,
                user_id, session_id, node_name, group_id, new_size, public_key_hash, passphrase)
...
def get_handler(environ):
    ...
    try:
        c = db.cursor(conn)
        resp = db.read_max_post_size(c,
                user_id, session_id, node_name, group_id, owner_id, passphrase)
